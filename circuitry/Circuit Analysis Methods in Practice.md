...
![[Pasted image 20260916154349.png|700]]

> [!NOTE] 对称性
> 电阻矩阵是对称的：$R_{kj} = R_{jk}$（共享电阻对两网孔的值相同）。这与节点法电导矩阵的对称性如出一辙。

### 3.5 含电流源的网孔法：直接法与超网孔 (Supermesh)

> [!IMPORTANT] 为什么电流源会给网孔法带来麻烦？
> 网孔法的核心是写 KVL，而 KVL 需要知道每个元件的电压。**电流源的电压无法由电流直接求出**（欧姆定律不适用）——它的电压由外电路决定。这与节点法里"电压源的电流未知"是完美的对偶。
>
> 两种应对策略：
> 1. **直接法 (Direct Method)**：引入电流源两端的电压 $v$ 作为额外变量，然后对两个网孔各写 KVL，再补一个 KCL 约束（电流源电流已知）。
> 2. **超网孔法 (Supermesh Method)**：把共享电流源的两个网孔**合并**成一个"超网孔"，在超网孔的外边界上写 KVL（跳过电流源），再补一个 KCL 约束。不引入新变量。

#### 情形一：电流源只属于一个网孔

这是最简单的情况——**该网孔的电流直接被电流源锁定**：

- 若电流源方向与网孔电流方向一致，则 $I_k = I_S$
- 若相反，则 $I_k = -I_S$

该网孔不需要写 KVL（电流已知），未知数直接减少一个。

> [!EXAMPLE] 单网孔电流源
> 若左网孔中有一个 6 A 电流源（方向与顺时针相反），则直接有 $I_1 = -6\,\text{A}$，只需对右网孔列一个 KVL 方程。

#### 情形二：电流源被两个网孔共享 → 超网孔

当电流源跨在两个网孔之间（两网孔共享一条含电流源的支路），两个网孔电流都不直接已知，此时用**超网孔 (Supermesh)**：

> [!DEFINITION] 超网孔 (Supermesh)
> 将共享电流源的两个网孔合并，**去掉中间的电流源及其串联元件**，形成一个更大的回路——这个大回路就叫"超网孔"。沿超网孔的外边界写 KVL，再用 KCL 补充两网孔电流的约束关系。

**超网孔的三个性质**：
1. 电流源提供了一个约束方程：$I_i - I_j = I_S$（取决于方向）
2. 超网孔本身没有"自己的电流"——它只是两个网孔电流的联合
3. 超网孔需要**同时使用 KVL 和 KCL**（与超节点对偶）

#### 超网孔法完整例题

> [!EXAMPLE] 双网孔 + 共享电流源
> 电路：左网孔含 $20\,\text{V}$ 电压源、$10\,\Omega$ 电阻；右网孔含 $4\,\Omega$ 电阻；两网孔之间共享一条支路：$6\,\text{A}$ 电流源串联 $2\,\Omega$ 电阻。求两网孔电流。
>
> **Step 1 — 识别超网孔**：$6\,\text{A}$ 电流源被网孔 1 和网孔 2 共享 → 二者合并为超网孔。
>
> **Step 2 — 对超网孔写 KVL**（沿外边界绕行，跳过中间的电流源与串联电阻）：
> $$-20 + 10\,I_1 + 4\,I_2 = 0 \quad\Rightarrow\quad 10I_1 + 4I_2 = 20$$
> （概念上：$20\,\text{V}$ 升 → $10\,\Omega$ 降 → $4\,\Omega$ 降 → 回到起点）
>
> **Step 3 — 补 KCL 约束**（在电流源所在支路上，两网孔电流的代数和等于电流源电流）：
> 设电流源从网孔 1 流向网孔 2，则 $I_1 - I_2 = 6$。
>
> **Step 4 — 联立求解**：
> $$\begin{cases} 10I_1 + 4I_2 = 20 \\ I_1 - I_2 = 6 \end{cases}$$
> 由第 2 式：$I_1 = I_2 + 6$，代入第 1 式：
> $$10(I_2+6) + 4I_2 = 20 \Rightarrow 14I_2 = -40 \Rightarrow \boxed{I_2 = -\dfrac{20}{7} \approx -2.86\,\text{A}}$$
> $$\boxed{I_1 = 6 - \dfrac{20}{7} = \dfrac{22}{7} \approx 3.14\,\text{A}}$$
>
> $I_2$ 为负表示实际方向与假设的顺时针相反——这在含电流源的电路中很常见。

> [!TIP] 直接法验证（可选）
> 设电流源两端电压为 $v$（上正下负），分别对两个网孔写 KVL：
> - Mesh 1：$-20 + 10I_1 + v + 2I_1 = 0$ → $12I_1 + v = 20$
> - Mesh 2：$-2I_1 - v + 4I_2 = 0$ → $-2I_1 + 4I_2 - v = 0$
> - 约束：$I_1 - I_2 = 6$
>
> 前两式相加消去 $v$：$10I_1 + 4I_2 = 20$，与超网孔法得到的 KVL 完全一致 ✓
>
> 直接法更直观但多一个变量；超网孔法变量少但需要"跳过去"的思维跳跃。**结果完全等价**，选你顺手的即可。

#### 超网孔 vs 超节点：完美对偶

| | 节点法 (KCL) | 网孔法 (KVL) |
| :--- | :--- | :--- |
| **麻烦制造者** | 电压源（电流未知） | 电流源（电压未知） |
| **简单情形** | 一端接地 → $e = V_S$ | 只属一网孔 → $I = I_S$ |
| **复杂情形** | 跨两节点 → **超节点** | 跨两网孔 → **超网孔** |
| **做法** | 合并两节点写 KVL 约束 + 合并写 KCL | 合并两网孔写 KCL 约束 + 合并写 KVL |
| **方程数** | 超节点 1 个 KCL + 1 个 KVL 约束 | 超网孔 1 个 KVL + 1 个 KCL 约束 |

> [!WARNING] 常见错误
> - **忘记补约束方程**：只写了超网孔 KVL，忘了补 $I_i - I_j = I_S$ 的 KCL 约束 → 方程数不够。
> - **电流源方向搞反**：$I_1 - I_2 = I_S$ 还是 $I_2 - I_1 = I_S$？画个小箭头标清楚。
> - **超网孔 KVL 漏元件**：沿外边界走一圈，遇到的每个电阻/电压源都要算进去，别漏了不与电流源串联的支路。
...
- [[Source Transformation|电源变换]] —— 第三种化简武器：源形态互换 + 级联化简（ECE 2001 Lecture 3）
