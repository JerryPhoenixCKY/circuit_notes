...
> [!NOTE] 课件把这件事讲得更清楚：功率传输有两个**不同**的优化目标
> | 目标 | 固定什么 | 调什么 | 典型领域 |
> | :--- | :--- | :--- | :--- |
> | **效率最大** (efficiency maximization) | 固定负载，调整系统 | 源/线路参数 | 电力系统 (power system) |
> | **功率最大** (maximum power transfer) | 固定系统，调整负载 | $R_L$ | 通信系统 (communication system) |
>
> **两者不能同时最优**——所以要先问清楚"这题到底在优化哪一个"。
> 本定理处理的是第二种。
...
**课件原图**（把 $P_L(R_L)$ 当作函数来"画图求极值"，与上面的解析求导互为印证）：

![[l3_max_power_curve.jpeg]]
...
>
> 课件用商法则 $\left(\frac{f}{g}\right)'=\frac{f'g-fg'}{g^2}$ 对
> $P_L=\dfrac{V_{Th}^2R_L}{(R_{Th}+R_L)^2}$ 求导，得到与 §一 相同的结论——
> **解析法与图解法在这里完全一致**。
...
> [!TIP] 顺带回答一个工程问题：实际电源什么时候"像理想源"？
> 课件在 Source Modeling 一节给出的结论（详见 [[Practical Two-Terminal Elements|实际二端元件]]）：
> - **实际电压源**（$V_s$ 串 $R_s$）：$R_L \gg R_s$ 时接近理想 —— 因为 $R_s$ 上的分压可以忽略
> - **实际电流源**（$I_s$ 并 $R_p$）：$R_L \ll R_p$ 时接近理想 —— 因为分流几乎全走负载
>
> **注意这与最大功率条件正好相反**：想让电源"理想"，就要**故意失配**。
> 理想源和最大功率传输，在工程上是两个互相拉扯的目标。
