...
### 1.1 电源建模 (Source Modeling)：什么时候"像"理想源？

课件用**电压源/电流源两条对偶路线**回答了这个问题：

![[l3_source_modeling.jpeg]]

| | 实际电压源（$V_s$ 串 $R_s$） | 实际电流源（$I_s$ 并 $R_p$） |
| :--- | :--- | :--- |
| 内部结构 | $R_s$ 与 $R_L$ **分压** | $R_p$ 与 $R_L$ **分流** |
| 何时接近理想 | $R_L \gg R_s$（$R_s$ 上分压可忽略） | $R_L \ll R_p$（分流几乎全给负载） |
| 极限 | $R_s \to 0$ | $R_p \to \infty$ |
| 直觉 | **负载越轻，端电压越稳** | **负载越重，输出电流越稳** |

> [!TIP] 一个值得记住的对照
> **"接近理想源"与"最大功率传输"是相反的目标。**
> 前者要求 $R_L \gg R_s$（故意失配，效率高）；
> 后者要求 $R_L = R_s$（严格匹配，但效率只有 50%）。
> 见 [[Maximum Power Transfer Theorem|最大功率传输定理]]。
>
> 这也说明：**同一个电路，问"功率最大"和问"效率最高"会得到不同答案**——
> 做题前先确认题目在优化哪个量。
