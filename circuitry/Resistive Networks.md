...
| **基本节点** | Essential Node | 连接 **三条及以上** 支路的节点（节点分析只需对基本节点列方程，非基本节点只是导线上的点，电压与相邻节点相同） |
| **基本支路** | Essential Branch | 连接两个基本节点、中间不经过其他基本节点的一段电路（可能包含多个串联元件） |

> [!TIP] 为什么要区分"基本"与"非基本"？
> 非基本节点只连两条支路——它就是导线上的一个点，两边电流必然相等、电压必然相同。做节点分析时，**只需对 $n_e-1$ 个基本节点写 KCL**，非基本节点可以跳过（它们不提供独立方程，反而增加未知数）。
>
> 同理，基本支路数 $b_e$ 比总支路数 $b$ 少——因为串联元件合并成了一条基本支路。网孔法的"网孔电流"本质上就是绕基本支路走的。
