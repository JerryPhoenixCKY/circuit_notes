...
| 状态变量 | $v_C$ 或 $i_L$ | $v_C$ **和** $i_L$ |
| ODE 阶数 | 1 | 2 |
| 特征方程 | 一次（1 个实根） | 二次（2 个根，可实/可复） |
| 响应形态 | 单调指数 | 衰减/振荡/临界 |
| 时间常数 | $\tau = RC$ 或 $L/R$ | $\omega_0 = 1/\sqrt{LC}$, $\zeta$ |
...
结合 $i_L = -C\,\dfrac{dv_C}{dt}$（电容放电方向），得：
...
> [!WARNING] 振荡需要 L 和 C
> 只有同时存在电感和电容时，能量才能在电场与磁场间交换、产生振荡。两个电容（或两个电感）之间只能通过电阻传递能量，过程中能量被消耗，无法"倒手"→ 无振荡。两个同类型储能元件的二阶电路永远过阻尼。
...
> $\zeta > 1$ → **过阻尼**。
>
> $s_{1,2} = -\zeta\omega_0 \pm \omega_0\sqrt{\zeta^2-1} = -2000 \pm 1010$
>
> $s_1 = -990\,\text{s}^{-1}$, $s_2 = -3010\,\text{s}^{-1}$
>
> $v_C(t) = A_1 e^{-990t} + A_2 e^{-3010t}$
>
> 由 $v_C(0) = 10$ → $A_1 + A_2 = 10$；由 $i_L(0) = 0$ → $\dot{v}_C(0) = 0$ → $-990A_1 - 3010A_2 = 0$
>
> 解得 $A_1 = 14.47$, $A_2 = -4.47$。
>
> $$v_C(t) = 14.47\,e^{-990t} - 4.47\,e^{-3010t}\,\text{V}$$
...
> - $Q = \frac{1}{2\zeta}$（品质因数 = 阻尼比的倒数）
> - $BW = \frac{\omega_0}{Q} = 2\zeta\omega_0$（带宽 = 衰减率）
>
