...
- **能量**：需要持续供电（SRAM）或不需要（DRAM/Filash — 靠电荷/浮栅）
- **正反馈**：把输出拉回输入，形成双稳态
...
```mermaid
graph LR
    D["D"] --> M["主锁存器<br/>(CLK=1 透明)"]
    M --> S["从锁存器<br/>(CLK=0 透明)"]
    S --> Q["Q"]
```

- **正边沿触发**：CLK 从 0→1 时，Q 捕获 D 的当前值
- 期间 (CLK=1) 的变化被主锁存器跟踪；时钟下降沿才传给从锁存器
...
> 触发器 → SRAM → DRAM 是一段"存储原理"的抽象降级：触发器的**正反馈**保证了速度但占 6 个晶体管；DRAM 用**电容**只占 1 个晶体管，但电荷会漏需要刷新——这是**面积/功耗/速度**三者的经典工程折中。
...
| 亚稳态 | 系统接近不稳定平衡点（类似 RLC 临界阻尼的"徘徊"） |
