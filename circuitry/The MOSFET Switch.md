...
| 两 nMOS **串联** 下拉 + 上拉 | **NAND** | 任一断开（"1"）→ 输出 "1"；皆闭合（"0,0"）... 实际：仅当两输入皆 "1" 时下拉通，输出 "0" |
| 两 nMOS **并联** 下拉 + 上拉 | **NOR** | 任一闭合（"0"）→ 输出 "0"；皆断开 → 输出 "1" |
| 串/并组合 | **复合门 (compound gate)** | 例如 $(A\cdot B)+C$：A、B 串联后再与 C 并联 |
...
满足 $[0,0.5]\subset[0,V_{IL}]$ 且 $[4.5,5]\subset[V_{IH},5]$ → **our inverter satisfies this**。不满足的例子：$V_{OL}=0.5\,\text{V} < V_{IL}=1.5\,\text{V}$（"0" 区间被错读成 "1"）→ 破坏静态约束。
