...
### 2.4 不平衡电桥：用戴维南定理算检流计电流（ECE 2001 Lecture 3）

平衡时检流计电流为 0，**但工程上更常见的是"不平衡"情形**——正是这一点让电桥能当传感器用：
**电压分压比的任何微小差异都会产生电流**，于是阻值变化被转换成可测电流。

课件给出的完整解法（用 [[Thevenin's Theorem|戴维南定理]] 把电桥从检流计两端看进去化简）：

![[l3_bridge_unbalanced.jpeg]]

**第一步：求检流计两端的戴维南电阻**（独立源置零后，从 a–b 看进去）

电桥被分成左右两条并联支路，每条支路内部是两个电阻串联：

$$R_{Th} = (3 \parallel 1) + (0.4 \parallel 0.6)\ \text{k}\Omega
= \frac{3\times1}{3+1} + \frac{0.4\times0.6}{0.4+0.6}
= 0.75 + 0.24 = \boxed{0.99\ \text{k}\Omega}$$

**第二步：求开路电压 $V_{Th}$**（两个分压器输出之差）

$$v_1 = 220\times\frac{1}{3+1} = 55\ \text{V}, \qquad
v_2 = 220\times\frac{0.6}{0.4+0.6} = 132\ \text{V}$$

$$V_{Th} = v_1 - v_2 = 55 - 132 = \boxed{-77\ \text{V}}$$

**第三步：接回检流计**（内阻 $R_G = 40\ \Omega = 0.04\ \text{k}\Omega$）

$$I_G = \frac{V_{Th}}{R_{Th} + R_G} = \frac{-77}{0.99 + 0.04} \approx -74.76\ \text{mA}$$

> [!IMPORTANT] 负号的正确读法
> $I_G<0$ 表示**实际电流方向与假设方向相反**（即从 b 流向 a），
> **不是"算错了"**。这正是 [[Thevenin's Theorem|戴维南定理 §4.3]] 讨论过的
> "负 $V_{Th}$ 只表示极性与参考方向相反"的实例。
>
> **工程意义**：检流计内阻 $R_G=40\ \Omega$ 相比 $R_{Th}=0.99\ \text{k}\Omega$ 很小，
> 所以 $I_G \approx V_{Th}/R_{Th}$——**电桥的输出电流主要由失衡电压决定**，
> 这就是传感器电桥能直接驱动负载/放大器的原因。
